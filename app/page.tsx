...
export default function Home() {
  return (
    <div id="top" className="relative">
      ...
      <main>
        <section className="section-shell relative pb-16 pt-10 md:pb-20 md:pt-[3.5rem]">
          <Container className="grid items-center gap-10 lg:grid-cols-[minmax(0,1fr)_34rem] xl:grid-cols-[minmax(0,1fr)_36rem] xl:gap-12">
            <div className="max-w-[41rem]">
              ...
              <h1 className="text-glow mt-6 max-w-[12ch] font-[var(--font-display)] text-[2.55rem] font-semibold leading-[0.98] tracking-[-0.04em] text-white sm:text-[3.45rem] lg:text-[4.05rem]">
              ...
              <p className="mt-5 max-w-[35rem] text-base leading-7 text-[var(--color-muted)] sm:text-[1.05rem]">
              ...
              <div className="mt-4 flex flex-col gap-2 text-sm text-[var(--color-muted)] sm:flex-row sm:flex-wrap sm:items-center sm:gap-5">
                {trustNotes.map((note) => (
              ...
              <div className="mt-8 grid gap-4 sm:grid-cols-3">
                {highlights.map((item) => (
                  <SurfaceCard
                    key={item.label}
                    className="min-h-[9.75rem] border-[rgba(132,164,218,0.14)] bg-[linear-gradient(180deg,rgba(10,22,39,0.9),rgba(8,16,29,0.78))] px-5 py-5 shadow-[0_18px_44px_rgba(0,0,0,0.28)] hover:border-[rgba(90,152,255,0.28)] hover:shadow-[0_22px_54px_rgba(7,18,34,0.46)]"
                  >
                    <div className="text-[11px] font-medium uppercase tracking-[0.26em] text-[var(--color-muted)]">
                      {item.label}
                    </div>
                    <div className="mt-4 text-[2rem] font-semibold leading-none text-white">
                      {item.value}
                    </div>
                    <div className="mt-2.5 text-sm leading-6 text-[var(--color-muted)]">
                      {item.detail}
                    </div>
                  </SurfaceCard>
                ))}
          ...
          <Container>
            ...
            <div className="mt-12 grid gap-6 md:grid-cols-3">
              {processSteps.map((step, index) => (
                <SurfaceCard key={step.title} className="group h-full px-7 py-8">
        ...
        <section id="markets" className="section-shell py-20 md:py-24">
          <Container>
            ...
            <div className="mt-12 grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
              {markets.map((market) => (
                <SurfaceCard key={market.state} className="px-6 py-6">
          ...
          <Container className="grid gap-10 lg:grid-cols-[0.8fr_1.2fr] lg:items-start">
            ...
            <div className="grid gap-6 md:grid-cols-2">
              {differentiators.map((item) => (
                <SurfaceCard key={item.title} className="h-full px-7 py-7">
                  <h3 className="font-[var(--font-display)] text-xl font-semibold text-white">
                    {item.title}
                  </h3>
